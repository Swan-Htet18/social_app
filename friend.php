...
<ul class="list-group">
	<?php
		...
		$fri_sql=mysqli_query($conn,"SELECT * FROM user WHERE id!='".$_SESSION['id']."'LIMIT $start,$limit");
		while($fri=mysqli_fetch_assoc($fri_sql))
		{
	?>
  <li class="list-group-item">
  	<a href="" class="card-link text-dark">
  	<b class="mr-2"><?= $fri['name']?></b>
  	<small> Live in: <?=$fri['address'] ?></small>
  	<div class="float-right">
  	<img src="img/<?= $fri['photo'] ?>" class="rounded-circle" width="50px" height="50px">
  	</a>
...
<?php } for($i=1;$i<=$page;$i++) { ?>
    <li class="page-item"><a class="page-link" href="?id=<?php echo $i;?>"><?php
		 echo $i;?></a></li>
<?php } if($id < $page) { ?>
